...
def detect(cfgfile, weightfile, imgfile):
    ...
    save_path = imgfile.replace('.jpg', '_output.jpg').replace('dataset', 'results')
...
def detect_skimage(cfgfile, weightfile, imgfile):
    ...
    class_names = load_class_names(namesfile)
    save_path = imgfile.split('/')[0].strip()
    plot_boxes_cv2(img, boxes, savename=savepath+'_output.jpg', class_names=class_names)
...
if __name__ == '__main__':
    if len(sys.argv) == 5:
        ...
        for imgfile in test_files[:-1]:
            detect(cfgfile, weightfile, test_folder+imgfile)
